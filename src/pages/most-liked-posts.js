...
class MostLikedPostsPage extends React.Component {
  static async fetchData(router, store, client) {
    const triggers = new ActionsTrigger(client, store.dispatch);

    await triggers.loadMostLikedPosts({ ...router.location.query, limit: LOAD_MORE_LIMIT });
  }
  ...
  componentWillReceiveProps(nextProps) {
    if (!isEqual(this.props.location.query, nextProps.location.query)) {
      ...
      this.triggers.loadMostLikedPosts(query);
    }
  }
  ...
  handleForceLoadPosts = async () => {
    ...
    const res = await this.triggers.loadMostLikedPosts(query);
  ...
  render() {
    return (
      <BestPostsBasePage
        {...this.props}
        title="Most Liked Posts of"
        loadingInProgresss={this.props.ui.getIn(['progress', 'loadMostLikedPostsInProgress'])}
...
const selector = createSelector(
  ...
  state => state.get('most_liked_posts'),
