...
import { posts, mostFavouritedPosts } from '../actions';

const initialState = i.List([]);

export default function reducer(state = initialState, action) {
  switch (action.type) {
    case mostFavouritedPosts.ADD_POSTS_TO_MOST_FAVOURITED_POSTS: {
    ...
    case mostFavouritedPosts.SET_MOST_FAVOURITED_POSTS: {
      const postIds = action.payload.posts.map(post => post.id);
